...
class GpxWriter(outputStream: OutputStream) : TrackWriter {
    ...
    override fun writeHeader() {
        ...
        recordingSettings?.let { settings ->
            ...
            writer.write("<sj:driverThresholds>\n")
            writer.write("<sj:hardBrakeFwdMax>${"%.3f".format(dt.hardBrakeFwdMax)}</sj:hardBrakeFwdMax>\n")
            writer.write("<sj:hardAccelFwdMax>${"%.3f".format(dt.hardAccelFwdMax)}</sj:hardAccelFwdMax>\n")
            writer.write("<sj:swerveLatMax>${"%.3f".format(dt.swerveLatMax)}</sj:swerveLatMax>\n")
            writer.write("<sj:aggressiveCornerLatMax>${"%.3f".format(dt.aggressiveCornerLatMax)}</sj:aggressiveCornerLatMax>\n")
            writer.write("<sj:aggressiveCornerDCourse>${"%.3f".format(dt.aggressiveCornerDCourse)}</sj:aggressiveCornerDCourse>\n")
            writer.write("<sj:minSpeedKmph>${"%.3f".format(dt.minSpeedKmph)}</sj:minSpeedKmph>\n")
            writer.write("<sj:movingAvgWindow>${dt.movingAvgWindow}</sj:movingAvgWindow>\n")
            writer.write("<sj:reactionTimeBrakeMax>${"%.3f".format(dt.reactionTimeBrakeMax)}</sj:reactionTimeBrakeMax>\n")
            writer.write("<sj:reactionTimeLatMax>${"%.3f".format(dt.reactionTimeLatMax)}</sj:reactionTimeLatMax>\n")
            writer.write("<sj:smoothnessRmsMax>${"%.3f".format(dt.smoothnessRmsMax)}</sj:smoothnessRmsMax>\n")
            writer.write("<sj:fallLeanAngle>${"%.3f".format(dt.fallLeanAngle)}</sj:fallLeanAngle>\n")
            writer.write("</sj:driverThresholds>\n")
            writer.write("</sj:recordingSettings>\n")
            writer.write("</extensions>\n")
            writer.write("</metadata>\n")
